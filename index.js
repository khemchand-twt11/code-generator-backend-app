...
app.post('/check', async (req, res) => {
  ...
  try {
    const qualityCheckResult = await checkCodeQuality(code, parameters)
      ...
      messages: [
        {
          role: 'system',
          content:
            'respond as a code converter from one language to another. give only the code nothing else',
        },
        {
          role: 'user',
          content: `Translate the following JavaScript code to ${language}: \n\n${code}`,
        },
      ],
      max_tokens: 150,
      temperature: 0.6,
  ...
  const response = await axios.post(
    'https://api.openai.com/v1/chat/completions',
    {
      messages: [
        {
          role: 'system',
          content: 'You are a user requesting code debugging.',
        },
        {
          role: 'user',
          content: `Debug the following JavaScript code: \n\n${code}`,
        },
      ],
      max_tokens: 150,
      temperature: 0.6,
  ...
  const response = await axios.post(
    'https://api.openai.com/v1/chat/completions',
    {
      messages: [
        {
          role: 'system',
          content: 'You are a user requesting code quality check.',
        },
        {
          role: 'user',
          content: `Evaluate the following JavaScript code: \n\n${code}\n\nBased on the parameters: ${parameters}`,
        },
      ],
      max_tokens: 150,
      temperature: 0.6,
